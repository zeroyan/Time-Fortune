...
function Game() {
    ...
    function addAction() {
        ...
        $('#putBufferfly').click(function(){
            ...
            if (current_play.bufferfly > 0) {
				...
				sendPutButterflyPush(current_player_turnId + "_" +  bf_id,res);
        ...
        $('#backHistory').click(function(){
			...
			//1、判断用户是否有时光回溯能力
			var bf_queue_test = current_player_turnId + '_';	//验证蝴蝶队列中是否有符合条件的蝴蝶
			var bf_array = [];		//时光回溯可选择的蝴蝶
			
			for(var i = 0;i < bf_queue.length;i++){
				if(bf_queue[i].substring(0,2) == bf_queue_test){
					bf_array.push(bf_queue[i]);
				}
			}
			
			console.log(bf_array);
			console.log(playerTimeHistoryNumber);
			
            if(bf_array != null && bf_array.length > 0 && playerTimeHistoryNumber > 0){
                var backGameInfoList = bf_cache[current_player_turnId];
                ...
                for (var i = 0;i < bf_array.length;i++){
                    ...
                    $(_this_bf).click(function(){
                        ...
                        var playerid_bfid = current_player_turnId + '_' + choseBfId;
                        //console.log("playerid_bfid: ", current_player_turnId + '_' + choseBfId);
                        backGameInfo = sendBackHistoryPush(playerid_bfid);
						//console.log(backGameInfo['player']);
						...
						console.log(backGameInfo['bfImageSet']);
    ...
    function getCurrentGameInfo(){
        ...
        var bf_list = $("div[id^='bfimg']:visible");
        var bf_id_list = [];
        for (var i=0; i<bf_list.length; i++){
            bf_id_list.push(bf_list[i].id);
        }
        info['bfImageSet'] = bf_id_list.join(',');
        return info;
    }
...
//向后台发送放置蝴蝶请求
function sendPutButterflyPush(playerBfId,res){
    //console.log(res);

	var data = {};
	data.BfId = playerBfId;
	data.Ratio = res.ratio + "";
	data.RoundId = res.roundId + "";
	data.Players = JSON.stringify(res.player);

	data.BfImageSet = res.bfImageSet;
	
	var dataStr = JSON.stringify(data);
	
	$.post("http://47.92.170.159:8080/api/putBufferfly", dataStr,"jsonp");
	
	
}

//向后台发送时光回溯请求
function sendBackHistoryPush(playerBfId){
	var info = {};
    info['bflmageSet'] = [];

	$.ajax({
        type:"get",
        url : "http://47.92.170.159:8080/api/getButterfly",
        data:{
            BfId : playerBfId
        },
		async : false,
        ...
        success:function (data) {
			...
			info['ratio'] = data.Ratio;
			info['player'] = JSON.parse(data.Players);
            //console.log("info['player']: ", info['player']);
            info['bfImageSet'] = data.BfImageSet;
        },
		error:function(data,type, err){
			//alert("error data = " + data.getResponseHeader());
			console.log(data);
